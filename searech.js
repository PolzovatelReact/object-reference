...
let search = function () {
  ...
  if (result.length > 0) {
    ...
    localStorage.setItem("lastSearch", ""); // не сохранять если отрицательный результат
  }
};

window.onload = function () {
  //// onload наступает событие когда загрузилась вся страница
  let lastSearch = localStorage.getItem("lastSearch");
  if (lastSearch) {
    document.getElementById("input-search").value = lastSearch; //input-search сохранение ввода в поле input
